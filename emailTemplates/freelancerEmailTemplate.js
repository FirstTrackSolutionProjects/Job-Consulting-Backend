...
/**
 * Generates an HTML email template for internal authority notification of a Freelancer application.
...
const generateEmailHTMLTemplate = (data) => {
  const {
    ...
    skills, experience, interestedField, portfolio, resume,
    loanAmount, purpose,
    accountHolderName, bankName, accountNumber, ifsc,
    photo, aadharFile, panFile, bankPassbook
  } = data;
  ...
  <div style="font-family: Arial, sans-serif; color: #333;">
    ...
    <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; width: 100%;">
      <tbody>
        ...
        <tr><td colspan="2"><strong>Freelance Details</strong></td></tr>
        ${row('Skills', skills)}
        ${row('Experience', experience)}
        ${row('Interested Field', interestedField)}
        ${row('Portfolio URL', portfolio)}
        ${row('Resume', resume, true)}
        ...
        ${row('IFSC Code', ifsc)}
        ${row('Bank Passbook', bankPassbook, true)}
...
const freelancerEmailTemplate = async (data) => {
  try {
    const mailOptions = {
      from: process.env.CONTACT_EMAIL,
      to: process.env.ADMIN_EMAIL,
      subject: `Freelancer Application from ${data.fullName}`,
      html: generateEmailHTMLTemplate(data),
    };
    await contactTransporter.sendMail(mailOptions);
  } catch (error) {
    console.error('Error generating freelancer application email template:', error);
    throw new Error('Failed to send freelancer application email');
  }
};

module.exports = freelancerEmailTemplate;
