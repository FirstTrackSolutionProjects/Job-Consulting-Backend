...
/**
 * Generates an HTML email template for internal authority notification of a Mortgage Loan application.
...
const mortgageLoanApplicationEmailTemplate = async (data) => {
  try {
    const mailOptions = {
      from: process.env.CONTACT_EMAIL,
      to: process.env.ADMIN_EMAIL,
      subject: `Mortgage Loan Application from ${data.fullName}`,
      html: generateEmailHTMLTemplate(data),
    };
    await contactTransporter.sendMail(mailOptions);
  } catch (error) {
    console.error('Error generating mortgage loan application email template:', error);
    throw new Error('Failed to send mortgage loan application email');
  }
};

module.exports = mortgageLoanApplicationEmailTemplate;